...
export async function getUsers(req: Request, res: Response) {
  try {
    ...
    return res
      .status(500)
      .json({ error: 'Erreur interne lors de la récupération des utilisateurs.' });
  }
}
...
export async function getUser(req: Request<{ id: string }>, res: Response) {
  ...
  if (isNaN(userId) || userId <= 0) {
    return res
      .status(400)
      .json({ error: "ID utilisateur invalide. L'ID doit être un entier positif." });
  }
  ...
  try {
    ...
    return res
      .status(500)
      .json({ error: "Erreur interne lors de la récupération de l'utilisateur." });
  }
}
...
export async function deleteUser(req: Request<{ id: string }>, res: Response) {
  ...
  try {
    ...
    return res
      .status(500)
      .json({ error: "Erreur interne lors de la suppression de l'utilisateur." });
  }
}
...
export async function updateUser(req: Request<{ id: string }>, res: Response) {
  ...
  try {
    ...
    return res
      .status(500)
      .json({ error: "Erreur interne lors de la mise à jour de l'utilisateur." });
  }
}
